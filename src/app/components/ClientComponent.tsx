...
import { getList, getTypeList, getMultipleTypeList } from "../services/getData";
import { PokemonType, Type, TypesOfPokemon } from "../types/common";
interface Result {
...
const ClientComponent = ({ types }: ClientComponentProps) => {
  ...
  const getResult = useCallback((type: string[], page: number) => {
    ...
    if (len === 0) {
      ...
      getMultipleTypeList(type).then(async (result) => {
        setLoading(false);
        let res: PokemonType[] = [];
        for (let i = 0; i < result.length; i++) {
          const data = await result[i].json();
          const pokemons = data.pokemon.map((i: TypesOfPokemon) => i.pokemon);
